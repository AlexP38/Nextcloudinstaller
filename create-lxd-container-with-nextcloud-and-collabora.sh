...
lxc start $container
lxc exec $container cd /usr/share/keyrings
lxc exec $container wget https://collaboraoffice.com/downloads/gpg/collaboraonline-release-keyring.gpg
lxc exec $container cp collaboraonline.sources > /etc/apt/sources.list.d/collaboraonline.sources
...
lxc exec $container cp vhost.conf > /etc/apache2/sites-available/000-nextcloud.conf
...
if [ $revproxy != 'host' ]
then
lxc exec $revproxy cp vhost-reverse-proxy.conf > /etc/apache2/sites-available/000-nextcloud-container.conf
